def add_d_leaves(t, v):
    ...
    """
    "*** YOUR CODE HERE ***"
    def add_d_leaves_num(t, v, n):
      while n>0:
        t.branches.append(Tree(v))
        n-=1
    
    def add_leaves(t, v, n):
      if t.is_leaf():
        add_d_leaves_num(t, v, n)
      else:
        for t0 in t.branches:
          add_leaves(t0, v, n+1)
        add_d_leaves_num(t, v, n)
    return add_leaves(t, v, 0)
...
def has_path(t, target):
    ...
    "*** YOUR CODE HERE ***"
    def has_path_num(t, target, n):
      if t.label==target[n]:
        if n==len(target)-1:
          return True
        for t0 in t.branches:
          if has_path_num(t0,target,n+1):
            return True
    if has_path_num(t, target, 0):
      return True
    else:
      return False   
