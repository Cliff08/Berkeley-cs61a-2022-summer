...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        expressions_val = scheme_eval(expressions.rest.first, env)
        env.define(signature, expressions_val)
        return signature
        # END PROBLEM 4
        ...
        fun=do_lambda_form(Pair(signature.rest, expressions.rest), env)
        env.define(name, fun)
        return name
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while rest!=nil and is_scheme_true(result):
        first=rest.first
        rest=rest.rest
        result=scheme_eval(first, env)
    judge=is_scheme_true(result)
    if judge:
        if judge=='True' or judge=='true' or judge=='#t':
            return True
        elif judge=='false' or judge=='#f':
            return False
        else:
            return result
    else:
...
def do_or_form(expressions, env):
    ...
    first, rest = expressions.first, expressions.rest
    result=scheme_eval(first, env)
    while rest!=nil and is_scheme_false(result):
        first=rest.first
        rest=rest.rest
        result=scheme_eval(first, env)
    judge=is_scheme_false(result)
    if judge:
        return False
    else:
        if judge=='True' or judge=='true' or judge=='#t':
...
def do_define_macro(expressions, env):
    ...
    validate_formals(formals)
    body=expressions.rest
    env.define(expressions.first.first,MacroProcedure(formals, body, env))
