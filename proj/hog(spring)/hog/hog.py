...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls==0:
        a=hefty_hogs(player_score, opponent_score)
    else:
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0==score1:
        ...
        if last_leader==0:
            ...
            return 0,'Player 0 takes the lead by %d'%(k)
    else:
        k=score1-score0
        if last_leader==1:
            return 1,None
        else:
            return 1,'Player 1 takes the lead by %d'%(k)
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    def sum(*args):
        ...
        k=total_samples
        while k>0:
            sumation+=original_function(*args)
            k=k-1
        sumation=sumation/total_samples
        return sumation
    return sum
    # END PROBLEM 8
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    b=hefty_hogs(score, opponent_score)
    a=hefty_hogs_strategy(score, opponent_score, threshold, num_rolls)
    if hog_pile(score+b, opponent_score)!=0:
        return 0
    else:
        return a
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    a=hog_pile_strategy(score, opponent_score, 15, 2)
    return a  
    # END PROBLEM 12
