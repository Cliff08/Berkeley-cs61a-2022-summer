...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = False
        if player.hand!=[]:
            max_power=player.hand[0]
            k=0
            for i in range(len(player.hand)):
                if player.hand[i].power(opponent_card)>max_power.power(opponent_card):
                    max_power=player.hand[i]
                    k=i
            self.attack+=max_power.attack
            self.defense+=max_power.defense
            best_card=player.hand.pop(k)
