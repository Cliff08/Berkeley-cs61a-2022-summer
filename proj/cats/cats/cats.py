...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for val in word_list:
        recent_no=diff_function(typed_word,val,limit)
        if min_no<limit:
            if recent_no<min_no:
                min_no=recent_no
                min_val=val
...
def sphinx_swaps(start, goal, limit):
    ...
    start1=start[1:]
    goal1=goal[1:]
    return k+sphinx_swaps(start1,goal1,limit)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    while i<len2 and i<len1:
        if sofar[i]==prompt[i]:
            ...
            break
    dic1={'id': user_id, 'progress': i/len2}
    upload(dic1)
    return i/len2
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    len1=len(times_per_player[0])
    len0=len(times_per_player)
    i=0
    j=1
    while i<len0:
        j=1
...
def fastest_words(match):
    ...
    list1=match["times"]
    list2=match["words"]
    list3=[[] for i in player_indices]
    len0=len(match["words"])
    for j in word_indices:
        k=0
        min_val=list1[0][j]
        for i in player_indices:
            if min_val>list1[i][j]:
                min_val=list1[i][j]
                k=i
        list3[k].append(list2[j])
    return list3
    # END PROBLEM 10
