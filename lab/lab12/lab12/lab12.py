...
def add_trees(t1, t2):
    ...
    if length_t1<length_t2:
        new_branches=list(map(add_trees, t1_branches, t2_branches[:length_t1]))+t2_branches[length_t1:]
    elif length_t1>=length_t2:
        new_branches=list(map(add_trees, t1_branches[:length_t2], t2_branches))+t1_branches[length_t2:]
...
def intersection(lst_of_lsts):
    ...
    num=0
    k=[]
    len_lst_of_lsts=len(lst_of_lsts)
    lst0=lst_of_lsts[0]
    for j in range(len(lst0)):
        k.append(0)
        for i in range(len_lst_of_lsts):
            if lst0[j] in lst_of_lsts[i]:
                k[j]+=1
        if k[j]==len_lst_of_lsts and k[j] not in elements:
            elements.append(lst0[j])
    return elements
