...
class Buffer:
    ...
    def pop_first(self):
        ...
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        source_dy=list(self.source)
        result=None
        for i in range(len(source_dy)):
            if len(source_dy[i])>0:
                result=source_dy[i].pop(0)
                break
        self.source=iter(source_dy)
        return result
        # END PROBLEM 1
    ...
    def current(self):
        ...
        source_dy=list(self.source)
        source_dy2=source_dy[:]
        self.source=iter(source_dy2) 
        sul=iter(source_dy)
        while len(self.current_line)==0:
            try:
                # BEGIN PROBLEM 1
                self.current_line=next(sul)     
                # END PROBLEM 1
